...
function config_etcd {
  ...
  while true; do
    ...
    if [[ "$?" == 0 ]]; then
      ...
      /opt/bin/etcdctl --no-sync mk /coreos.com/network/config "{\"Network\":\"${FLANNEL_NET}\", \"Backend\": {\"Type\": \"vxlan\"}${FLANNEL_OTHER_NET_CONFIG}}"
      attempt=$((attempt+1))
      sleep 3
    fi
  done
